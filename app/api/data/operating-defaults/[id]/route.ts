...
export async function PATCH(
  ...
  try {
    ...
    const row = await prisma.aircraftOperatingDefault.update({
      where: { id },
      data: {
        costKey: parseOptionalString(body.costKey),
        annualAmount: parseOptionalDecimal(body.annualAmount),
...
export async function DELETE(
  ...
  try {
    await requireAdmin();
    const { id } = await params;
    await prisma.aircraftOperatingDefault.delete({ where: { id } });
