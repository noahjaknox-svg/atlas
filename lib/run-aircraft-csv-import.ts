...
export type AircraftCsvImportResult = {
...
async function upsertFbo(prisma: PrismaClient, airportId: string, fboName: string) {
...
async function upsertAircraftMaster(
  prisma: PrismaClient,
  spec: (typeof CANONICAL_AIRCRAFT)[0]
) {
...
/** Upsert reference data from parsed pro-forma CSV content. */
export async function importAircraftCsvParsed(
  prisma: PrismaClient,
  parsed: ParsedCsv
): Promise<AircraftCsvImportResult> {
  const masterIds = new Map<string, string>();

  for (const ap of AIRPORTS) {
    await upsertAirport(prisma, ap);
  }

  for (const spec of CANONICAL_AIRCRAFT) {
    const master = await upsertAircraftMaster(prisma, spec);
    ...
    for (const [rowLabel, costKey] of Object.entries(OPERATING_COST_ROWS)) {
      ...
      await upsertOperatingDefault(prisma, master.id, costKey, amount);
    }
  ...
  for (const provider of HANGAR_PROVIDERS) {
    ...
    if (provider.fboName) {
      const fbo = await upsertFbo(prisma, airport.id, provider.fboName);
      fboId = fbo.id;
    }
    ...
    for (const spec of CANONICAL_AIRCRAFT) {
      ...
      const ratePerSqft =
        provider.pricingMethod === "sqft_rate" && sqft && sqft > 0 ? quoted / sqft : null;
  ...
  const count = masterIds.size;
  return {
    aircraftTypes: count,
    message: `Imported ${count} aircraft types, hangar rates, and scenario templates from seed CSV.`,
  };
}

/** Upsert reference data from the bundled seed CSV (or optional path). */
export async function importAircraftCsvFromFile(
  prisma: PrismaClient,
  filePath?: string
): Promise<AircraftCsvImportResult> {
  return importAircraftCsvParsed(prisma, readAircraftCsvFile(filePath));
}

/** Upsert reference data from uploaded CSV text (same format as seed file). */
export async function importAircraftCsvFromContent(
  prisma: PrismaClient,
  content: string
): Promise<AircraftCsvImportResult> {
  return importAircraftCsvParsed(prisma, parseAircraftCsv(content));
}
